...
// CORS configuration  
app.use(cors({  
  origin: "https://passwordpro-eight.vercel.app", // Correct origin without trailing slash  
  credentials: true, // Optional: include credentials in CORS  
}));  
app.use(express.json());   
...
app.get("/api/passwords", async (req, res) => {  
  try {  
    const passwords = await Password.find();  
    res.json({ data: passwords });  // Standardized response  
  } catch (error) {  
    console.error("Error fetching passwords:", error);  
    res.status(500).json({ error: "Error fetching passwords" });  
  }  
